#!/usr/bin/env bash
#
# flash_xenon.sh -- flash a prebuilt Xenon sensor .hex onto a directly
...
HEX_FILE="${SCRIPT_DIR}/../prebuilt/xenon_sensor.hex"
...
if [ ! -f "${HEX_FILE}" ]; then
    ...
    echo "Make sure a prebuilt xenon_sensor.hex has been placed in prebuilt/." >&2
    exit 1
fi
...
echo
echo "Flash succeeded: xenon_sensor.hex programmed and verified."
